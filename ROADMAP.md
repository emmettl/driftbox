...
| Rack | An unpublished work in progress with 31 modules, patching UI, selectable live microphone/audio-interface input, EQ, keyboard/MIDI, tracker, sampler, a Combinator whose four rotaries drive any parameter anywhere and can be learned onto a hardware controller, a patchable VU Meter, undo, stereo cables, performance mode, offline export, retained groovebox playback and a metered four-machine Groovebox source with level, pan and mute strips. It is intended to join the published packages when complete: see [docs/RACK.md](docs/RACK.md) |
