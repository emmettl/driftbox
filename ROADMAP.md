...
| Rack | An unpublished work in progress with 28 modules, patching UI, keyboard/MIDI, tracker, sampler, a Combinator whose four rotaries drive any parameter anywhere and can be learned onto a hardware controller, performance mode, offline export, retained groovebox playback and a patchable four-machine Groovebox source. It is intended to join the published packages when complete: see [docs/RACK.md](docs/RACK.md) |
...
their 808, 909 and two 303s as patchable stereo sources, and return to the sequencer
without losing that document. Next, add rack-side editors over the retained song; do not
