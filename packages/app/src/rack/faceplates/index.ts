...
const FACEPLATES: Record<string, Entry> = {
  ...
  // Eight generated voices, a real held Alter button and two rows of voicing controls. Five rows keeps the
  // preview readable without making its rotaries smaller than the rest of the rack.
  'chord-player': { component: ChordPlayer, rows: 5 },
  ...
  // clipboard, instrument, send and effect editors. The authored PCF lane and complete
  // master-insert row need two more rows; keeping them visible is preferable to a modal.
  groovebox: { component: Groovebox, rows: 16 },
