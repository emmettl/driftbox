  ...
  const controls = () => (
    <>
      {/* Drawn from the point rather than from the filter, so the dot is under the finger even while the
          filter is still gliding towards it. Both faces get one; backface visibility makes the hand-off
          at the middle of the turn without leaving an unrotated overlay behind. */}
      {point && (
        <span
    ...
    >
      <div className="rk-pad-turn">
        ...
        {/* There is no patch panel behind an insert that lives outside the rack graph. The pad therefore
            has a deliberately unserious back side: the little creature living behind it. Both faces stay
            mounted so Back/Front is a physical turn rather than an image swap at the start of one. */}
        <div className="rk-pad-face rk-pad-face-back" aria-hidden={!flipped}>
